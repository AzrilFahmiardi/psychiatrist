    ...
    <form action="{{ route('pasien.update')}}" method="post">
        ...
        <div class="grid gap-6 mb-6 md:grid-cols-2">
            <div>
                ...
                <input type="text" id="nama" name="nama_lengkap" class="border-[1px] border-[#4F4F4F] text-[#4F4F4F] text-sm rounded-2xl focus:ring-blue-500 focus:border-blue-500 block w-full py-[1rem] px-4" placeholder="nama lengkap" value="{{ $user ? $user->nama_lengkap : '' }}" readonly />
            </div>
            <div class="relative">
                <label for="program_studi" class="block mb-2 text-sm font-semibold text-[1rem] text-[#155458]">Program studi</label>
                <select id="program_studi" name="program_studi" class="select2 border-[1px] border-[#4F4F4F] text-[#4F4F4F] text-sm rounded-2xl focus:ring-blue-500 focus:border-blue-500 block w-full py-[1rem] px-4 appearance-none bg-[url('{{ asset('images/down-arrow.svg') }}')] bg-no-repeat bg-right-[10px] bg-center" disabled>
            ...
            <div>
                ...
                <input type="number" id="company" name="semester"  min="1" max="14" class="border-[1px] border-[#4F4F4F] text-[#4F4F4F] text-sm rounded-2xl  block w-full py-[1rem] px-4 " value="{{ $user ? $user->semester : '' }}" placeholder="contoh : 6" readonly />
            </div>  
            <div>
                ...
                <div class="flex items-center mt-2 text-[#155458]">
                    <label for="laki-laki" class="inline-flex items-center mr-6">
                        <input type="radio" id="laki-laki" name="jenis_kelamin" value="laki-laki" class="form-radio border-[#155458] focus:ring-[#155458] text-[#155458]" {{ $user && $user->jenis_kelamin == 'laki-laki' ? 'checked' : '' }} disabled>
                        <span class="ml-2">Laki-laki</span>
                    </label>
                    <label for="perempuan" class="inline-flex items-center">
                        <input type="radio" id="perempuan" name="jenis_kelamin" value="perempuan" class="form-radio border-[#155458] focus:ring-[#155458] text-[#155458]" {{ $user && $user->jenis_kelamin == 'perempuan' ? 'checked' : '' }} disabled>
            ...
            <div>
                ...
                <input type="number" id="usia" name="usia"  min="0" class="border-[1px] border-[#4F4F4F] text-[#4F4F4F] text-sm rounded-2xl  block w-full py-[1rem] px-4  " value="{{ $user ? $user->usia : '' }}" placeholder="contoh : 20" readonly />
            </div>
            <div>
                <label for="no_hp" class="block mb-2 text-sm font-semibold text-[1rem] text-[#155458]">No. HP/WA</label>
                <input type="text" id="no_hp" name="no_hp" class="border-[1px] border-[#4F4F4F] text-[#4F4F4F] text-sm rounded-2xl  block w-full py-[1rem] px-4  " value="{{ $user ? $user->no_hp : '' }}" placeholder="contoh : 08****" readonly />
            </div>
            <div class="relative">
                <label for="departemen" class="block mb-2 text-sm font-semibold text-[1rem] text-[#155458]">Asal departemen</label>
                <select id="departemen" name="departemen" class="border-[1px] border-[#4F4F4F] text-[#4F4F4F] text-sm rounded-2xl focus:ring-blue-500 focus:border-blue-500 block w-full py-[1rem] px-4 appearance-none bg-[url('{{ asset('images/down-arrow.svg') }}')] bg-no-repeat bg-right-[10px] bg-center" disabled>
            ...
            <div>
                ...
                <div class="flex items-center mt-2 text-[#155458]">
                    <label for="psikolog" class="inline-flex items-center mr-6">
                        <input type="radio" id="psikolog" name="status_akses_layanan" value="psikolog" class="form-radio border-[#155458] focus:ring-[#155458] text-[#155458]" {{ $user && $user->status_akses_layanan == 'psikolog' ? 'checked' : '' }} disabled>
                        <span class="ml-2">Psikolog</span>
                    </label>
                    <label for="psikiater" class="inline-flex items-center mr-6">
                        <input type="radio" id="psikiater" name="status_akses_layanan" value="psikiater" class="form-radio border-[#155458] focus:ring-[#155458] text-[#155458]" {{ $user && $user->status_akses_layanan == 'psikiater' ? 'checked' : '' }} disabled>
                        <span class="ml-2">Psikiater</span>
                    </label>
                    <label for="belum" class="inline-flex items-center">
                        <input type="radio" id="belum" name="status_akses_layanan" value="belum pernah" class="form-radio border-[#155458] focus:ring-[#155458] text-[#155458]" {{ $user && $user->status_akses_layanan == 'belum pernah' ? 'checked' : '' }} disabled>
